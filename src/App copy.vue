...
<template>
  <div class="px-12 py-32 w-screen h-screen flex flex-col" style="overflow: hidden">
    ...
    <div class="center flex-1 w-0 py-32">
      <van-button icon="plus" class="rotate-90" type="primary">
        <span>启动</span>
      </van-button>
      <!-- <van-grid clickable class="rotate-90">
        <van-grid-item class="rotate-180" text="启动" icon="play-circle-o" @click="start" />
        <van-grid-item class="rotate-180" disabled text="停止" icon="pause-circle-o" @click="stop" />
        <van-grid-item class="rotate-180" text="重置" icon="replay" @click="reset" />
      </van-grid> -->
    </div>
    <div class="right">
      <div class="flex flex-row jusity-center items-center">
...
export default {
  ...
  watch: {
    running(val) {
      $delay(() => {
        let cmd = 'STOP';
        let v = ((Math.abs(50 - val) / 50) * (255 - this.offset)).toFixed(0);
        if (val > 50) {
          cmd = 'FORWARD';
        }
        if (val < 50) {
        ...
        this.sendMsg(cmd, parseInt(v) + this.offset);
      });
    },
    angles(val) {
      $delay(() => {
